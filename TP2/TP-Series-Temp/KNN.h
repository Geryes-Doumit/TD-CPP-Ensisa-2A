...
class KNN {
    ...
    public:
        ...
        double euclidean_distance(const vector<double>& timeSeries1, const vector<double>& timeSeries2) {
            ...
            if (size1 != size2) {
                cout << "Warning: time series have different sizes." << endl;
            }
        ...
        double dtw(const vector<double>& timeSeries1, const vector<double>& timeSeries2) {
            int size = timeSeries1.size();
            if (size != timeSeries2.size()) {
                cout << "Error: time series have different sizes." << endl;
                return -1;
            }

            vector<vector<double>> matrix = vector<vector<double>>(size + 1, vector<double>(size + 1, DBL_MAX));
            matrix[0][0] = 0;
